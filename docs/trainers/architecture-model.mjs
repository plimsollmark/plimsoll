...
export const nodes = {
  caller: {name: 'Caller program', sub: 'MCP server · gateway · Go client', x: 25, y: 96,
    ...
    holds: 'The credential used to call plimsolld. It selects a grant profile by name.', source: 'client/client.go'},
  ingress: {name: 'RPC entrance', sub: 'Authenticate, then decode', x: 285, y: 96,
    ...
    detail: 'RPC means remote procedure call: a program asks this server to run an operation. HTTP middleware checks the bearer and code:run scope before Connect reads the body. Raw bytes, decompressed bytes, read time, and concurrent decoding are bounded.',
    holds: 'Caller identity and procedure permissions. This credential is not the downstream API credential.', source: 'internal/rpc/auth.go'},
  ...
  operator: {name: 'Operator tools', sub: 'Configuration · probes · logs', x: 25, y: 292,
    ...
    detail: 'This box groups the operator’s configuration, log consumer, and monitoring tools. They are not a mandatory hop in a code execution. Startup, probes, and audit emission use different paths.',
  ...
  minter: {name: 'Credential source', sub: 'Mint JWT or obtain static token', x: 535, y: 292,
    role: 'Supplies the downstream API credential once per granted run.',
...
export const providers = {
  ...
  runsc: {provider: 'Docker · gVisor', engine: 'Node behind runsc', tier: 'kernel', channelShort: 'HTTP over a Unix socket',
    ...
    boundary: 'The kernel tier requires verified runsc configuration plus the startup behavioral checks. This is not runtime attestation.',
...
add('submit', 'caller', 'ingress', 'request', 'The program submits code',
  ...
  'A bearer credential is a secret token the caller presents to plimsolld to prove its identity. The model-written code is just request data at this point.',
...
add('submit-grant', 'caller', 'ingress', 'request', 'Select a profile by name',
  ...
  'The caller’s bearer token proves its identity to plimsolld. The chosen profile, “inventory-read”, defines separate API permissions held by the server. The item ID is an invented example.',
...
add('authenticate', 'ingress', 'service', 'request', 'Authenticate before decoding',
  'A decoded request plus the authenticated principal, which is the caller’s identity.',
  'HTTP middleware first checks the bearer and code:run scope, then bounds concurrent decoding. The handler validates the code or file bounds. Raw and decompressed body limits are separate protections.',
  'Principal { UserID: "client-a", Scopes: ["code:run"] }\n+ validated Request or ProjectRequest', 'internal/rpc/auth.go');
...
add('admit', 'service', 'limiter', 'request', 'Check isolation, then reserve capacity',
  'A request whose minimum tier is satisfied, keyed by authenticated caller.',
...
add('dispatch', 'limiter', 'provider', 'request', 'The handler dispatches the admitted run',
  'A plain Go Request or ProjectRequest passed to the selected Sandbox implementation.',
  'The limiter returns a release function to the handler, which calls the provider. This arrow abbreviates that control handoff; the limiter itself does not execute code. Build selected {{provider}} at startup.',
...
add('profile-denied', 'profiles', 'service', 'deny', 'Refuse this profile selection',
  ...
  'A valid plimsolld caller credential proves identity. It does not authorize every downstream grant. No token is minted and no provider is dispatched.',
